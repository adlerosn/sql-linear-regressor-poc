WITH R AS (
	select Year AS x, Height AS y from athlete_events
), T1 AS (
...
select 'athlete_events' AS tablename, 'Year' AS field_x, 'Height' AS field_y, slope, intercept from Tslope join Tintercept on (1=1)
;